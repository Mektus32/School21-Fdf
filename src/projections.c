...
/*   Updated: 2019/06/08 18:52:18 by ojessi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_horiz_line(int i, int j, t_line *line, t_map *map)
{
	...
	if (map->proj == 1)
	{
		iso(&line->start.x, &line->start.y, map->points[i][j].z * map->dz / 8);
		iso(&line->stop.x, &line->stop.y , map->points[i][j + 1].z * map->dz / 8);
	}
...
void	ft_vertic_line(int i, int j, t_line *line, t_map *map)
{
	...
	{
		iso(&line->start.x, &line->start.y, map->points[i][j].z * map->dz / 8);
		iso(&line->stop.x, &line->stop.y, map->points[i + 1][j].z * map->dz / 8);
	}
	ft_print_line(map ,line);
}
